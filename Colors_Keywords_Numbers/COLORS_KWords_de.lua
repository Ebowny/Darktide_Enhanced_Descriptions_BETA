...
-- COLORS_KWords_de.Ability_cd_rgb_de
-- COLORS_KWords_de.Cd_rgb_de
-- COLORS_KWords_de.Combat_ability_rgb_de
...
local function create_colored_keywords(config)
	...
	for color_setting, keywords in pairs(config) do
		...
		for name, text in pairs(keywords) do
			result[name .. "_rgb_de"] = iu_actit(text, color)
		end
	end
