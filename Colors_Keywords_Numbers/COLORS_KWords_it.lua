...
-- COLORS_KWords_it.Ability_cd_rgb_it
-- COLORS_KWords_it.Cd_rgb_it
-- COLORS_KWords_it.Combat_ability_rgb_it
...
local function create_colored_keywords(config)
	...
	for color_setting, keywords in pairs(config) do
		...
		for name, text in pairs(keywords) do
			result[name .. "_rgb_it"] = iu_actit(text, color)
		end
	end
